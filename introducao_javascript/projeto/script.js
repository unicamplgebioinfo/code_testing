...
carList.map((car, index) => {
  let carCard = window.document.getElementById("card")
  carCard.innerHTML += `
      <div class="col-md-4">
        <div class="card m-2">
          ...
          <div class="card-body">
            ...
            <a href="#" class="btn btn-secondary" onclick="zoomImg(${index})"><i class="bi bi-zoom-in"></i></a>
          </div>
        </div>
      </div>`
...
function zoomImg(index) {
  let carSelected = carList[index]
  document.getElementById("carName").innerHTML = carSelected.nome
  document.getElementById("carDesc").innerHTML = carSelected.descricao
  document.getElementById("imgModal").src = carSelected.img
...
function changeTheme() {
  ...
  let themeIcon = window.document.getElementById("themeIcon")
